...
\begin{itemize}
   \item{Intervention: } if reported by the oracle is lower than in the pool by significantly enough margin, i.e. $\frac{p*e}{u} < r$, where $r$ is some constant which is hard-wired into the protocol, e.g. $r = 0.95$, then the bank is providing \bc{} to restore the price
   \item{Payout: }
\end{itemize}
