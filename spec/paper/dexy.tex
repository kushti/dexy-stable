...
\begin{abstract}
In this paper, we consider a new stablecoin protocol design called \em{Dexy}. The protocol has two mandatory components, reference market~(in form of liquidity pool where anyone can 
trade base currency against stable coins), and an algorithmic central bank, which is responsible for new stablecoins issuance, and also for stabilizing stablecoin price in the markets via interventions, as central banks are doing for fiat currencies. 
\end{abstract}
