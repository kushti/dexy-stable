...
object OrdinaryLp {
  val ordinaryLpScript =
    """
      |// Ordinary LP contract used in Spectrum,
      |// taken from https://github.com/spectrum-finance/ergo-dex/blob/master/contracts/amm/cfmm/v1/n2t/Pool.sc
      |
      |{
  ...
  val ordinaryLpErgoTree: Values.ErgoTree = ScriptUtil.compile(Map(), ordinaryLpScript)
  val ordinaryLpAddress: String = getStringFromAddress(getAddressFromErgoTree(ordinaryLpErgoTree))
